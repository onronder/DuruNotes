...
void main() {
  group('ImportService Integration - Encryption & Indexing Verification', () {
    ...
    test('ImportService integration calls verify encryption and indexing flow', () async {
...
''');
      ...
      expect(importedNote.body.contains('integration between ImportService'), true);
      ...
      expect(encryptedTitle.length, greaterThan(0), reason: 'Encrypted title should not be empty');
      ...
      expect(decryptedTitle, importedNote.title, reason: 'Decryption should restore original title');
      ...
      expect(integrationTagResults.contains(importedNote.id), true,
        reason: 'Note should be findable by #integration tag');
      
      final testTagResults = testIndexer.findNotesByTag('test');
      expect(testTagResults.contains(importedNote.id), true,
      ...
      expect(importServiceResults.contains(importedNote.id), true,
      ...
      expect(indexStats['indexed_notes'], 1, reason: 'Should have 1 indexed note');
      expect(indexStats['total_tags'], greaterThanOrEqualTo(2), reason: 'Should have at least 2 tags');
      expect(indexStats['total_words'], greaterThan(0), reason: 'Should have indexed words');
      ...
      final invalidResult = await testImportService.importMarkdown(invalidFile);
      expect(invalidResult.isSuccess, false, reason: 'Empty file import should fail');
      
      // Verify system is still functional after error
      final postErrorNotes = await testDb.allNotes();
      expect(postErrorNotes.length, 1, reason: 'Should still have original note after error');
      
      final postErrorStats = testIndexer.getIndexStats();
      expect(postErrorStats['indexed_notes'], 1, reason: 'Index should be unchanged after error');

      // Clean up
      await testDb.close();
    });

    test('Multiple file import maintains encryption and indexing integrity', () async {
      ...
      for (final note in allNotes) {
        ...
        expect(encrypted1, isNot(equals(encrypted2)), 
          reason: 'Same content should encrypt differently due to random nonces');
      ...
      expect(firstResults.length, 1, reason: 'Should find exactly one note with #first tag');
      
      final secondResults = testIndexer.findNotesByTag('second');
      expect(secondResults.length, 1, reason: 'Should find exactly one note with #second tag');
      
      final thirdResults = testIndexer.findNotesByTag('third');
      expect(thirdResults.length, 1, reason: 'Should find exactly one note with #third tag');
      
      // Verify content-based search
      final uniqueResults = testIndexer.searchNotes('unique');
      expect(uniqueResults.length, 1, reason: 'Only first note contains "unique"');
      
      final contentResults = testIndexer.searchNotes('content');
      expect(contentResults.length, 2, reason: 'First and second notes contain "content"');
      
      // Verify index statistics
      final indexStats = testIndexer.getIndexStats();
      expect(indexStats['indexed_notes'], 3, reason: 'Should have 3 indexed notes');
      expect(indexStats['total_tags'], 3, reason: 'Should have 3 unique tags');
    ...
    test('Verify ImportService._createNoteWithValidation calls repository and indexer', () async {
...
''');
      ...
      expect(repositoryNotes.length, 1, 
        reason: 'NotesRepository.createOrUpdate should have been called to store the note');
      ...
      expect(searchResults.contains(storedNote.id), true,
        reason: 'NoteIndexer.indexNote should have been called to index the note');
      ...
      expect(validationTagResults.contains(storedNote.id), true,
      ...
      expect(linkResults.contains(storedNote.id), true,
      ...
      expect(decryptedNote, storedNote.body,
      ...
      expect(testIndexer.searchNotes('createNoteWithValidation').contains(storedNote.id), true,
