...
class TestReminderService extends BaseReminderService {
  TestReminderService(super.ref, super.plugin, super.db, {super.cryptoBox});
...
void main() {
  ...
  group('Migration v42: ReminderConfig encryption', () {
    test(
      'toCompanionWithEncryption encrypts title, body, and location_name',
      () async {
        // Arrange
  ...
  group('Migration v42: BaseReminderService decryption', () {
    test(
      'decryptReminderFields prefers encrypted data when available',
      () async {
        // Arrange
    ...
    test(
      'decryptReminderFields falls back to plaintext on decryption error',
      () async {
    ...
    test(
      'decryptReminderFields returns plaintext when no encrypted data',
      () async {
  ...
  group('Migration v42: Lazy encryption', () {
    ...
    test('ensureReminderEncrypted returns false when no CryptoBox', () async {
      ...
      final serviceWithoutCrypto = containerWithoutCrypto.read(
        _serviceNoCryptoProvider,
      );
      ...
      // Act
      final encrypted = await serviceWithoutCrypto.ensureReminderEncrypted(
        reminder!,
      );
    ...
    test('ensureReminderEncrypted handles user mismatch', () async {
      ...
      verify(
        mockLogger.warning(argThat(contains('Cannot encrypt reminder'))),
      ).called(1);
    });

    test(
      'getRemindersForNote triggers lazy encryption in background',
      () async {
        // Arrange
        reset(mockCrypto); // Reset to clear verification state
        reset(mockAnalytics);
