...
void main() {
  ...
  group('Snooze Limit Enforcement', () {
    test('should allow snoozing up to 5 times', () async {
      ...
      when(mockDb.snoozeReminder(any, any))
          .thenAnswer((_) async {});
    ...
    test('should show max snooze notification when limit reached', () async {
      ...
      when(mockDb.getReminderById(reminderId))
          .thenAnswer((_) async => reminder);
      when(mockReminderCoordinator.snoozeService)
          .thenReturn(snoozeService);
  ...
  group('Snooze Duration Options', () {
    test('should convert Duration to correct SnoozeDuration enum', () {
      ...
      expect(
        taskReminderBridge.testDurationToSnoozeDuration(const Duration(minutes: 3)),
        equals(SnoozeDuration.fiveMinutes),
      );
      expect(
        taskReminderBridge.testDurationToSnoozeDuration(const Duration(minutes: 8)),
        equals(SnoozeDuration.tenMinutes),
      );
      expect(
        taskReminderBridge.testDurationToSnoozeDuration(const Duration(minutes: 12)),
        equals(SnoozeDuration.fifteenMinutes),
      );
      expect(
        taskReminderBridge.testDurationToSnoozeDuration(const Duration(minutes: 25)),
        equals(SnoozeDuration.thirtyMinutes),
      );
      expect(
        taskReminderBridge.testDurationToSnoozeDuration(const Duration(minutes: 45)),
        equals(SnoozeDuration.oneHour),
      );
      expect(
        taskReminderBridge.testDurationToSnoozeDuration(const Duration(hours: 1, minutes: 30)),
        equals(SnoozeDuration.twoHours),
      );
      expect(
        taskReminderBridge.testDurationToSnoozeDuration(const Duration(hours: 10)),
  ...
  group('Snooze Persistence', () {
    test('should persist snooze count across app restarts', () async {
      ...
      // First snooze
      await snoozeService.snoozeReminder(reminderId, SnoozeDuration.fifteenMinutes);
