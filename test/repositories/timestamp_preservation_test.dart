...
void main() {
  ...
  group('Timestamp Preservation (Regression Tests for Production Bug)', () {
    ...
    test('CRITICAL: Sync download preserves remote created_at and updated_at', () async {
      ...
      final remoteCreatedAt = DateTime.utc(2025, 10, 28, 9, 0, 0); // Oct 28, 9 AM
      final remoteUpdatedAt = DateTime.utc(2025, 10, 28, 15, 30, 0); // Oct 28, 3:30 PM
      ...
      expect(created!.createdAt.toUtc().millisecondsSinceEpoch,
          remoteCreatedAt.millisecondsSinceEpoch,
          reason: 'created_at must match remote database timestamp (ignoring timezone)');
      expect(created.updatedAt.toUtc().millisecondsSinceEpoch,
          remoteUpdatedAt.millisecondsSinceEpoch,
          reason: 'updated_at must match remote database timestamp (ignoring timezone)');

      // Verify timestamps are preserved in database
      final retrieved = await harness.repository.getNoteById('note-from-remote');
      expect(retrieved!.createdAt.toUtc().millisecondsSinceEpoch,
          remoteCreatedAt.millisecondsSinceEpoch);
      expect(retrieved.updatedAt.toUtc().millisecondsSinceEpoch,
          remoteUpdatedAt.millisecondsSinceEpoch);
    });

    test('CRITICAL: Update without timestamp preserves existing updated_at', () async {
      ...
      final originalUpdatedAt = DateTime.utc(2025, 10, 27, 10, 0, 0); // Oct 27, 10 AM
      ...
      expect(note.updatedAt.toUtc().millisecondsSinceEpoch,
    ...
    test('CRITICAL: Fresh install preserves note age relationships', () async {
      ...
      final recentNoteDate = DateTime.utc(2025, 10, 29, 8, 0, 0); // Oct 29, 8 AM
      ...
      expect(oldNote!.createdAt.toUtc().millisecondsSinceEpoch,
          oldNoteDate.millisecondsSinceEpoch);
      expect(recentNote!.createdAt.toUtc().millisecondsSinceEpoch,
          recentNoteDate.millisecondsSinceEpoch);
      expect(oldNote.createdAt.millisecondsSinceEpoch < recentNote.createdAt.millisecondsSinceEpoch, isTrue,
          reason: 'Old note must be older than recent note');

      // Verify the age difference is preserved (2 days)
      final ageDifferenceMs = recentNote.createdAt.toUtc().millisecondsSinceEpoch -
          oldNote.createdAt.toUtc().millisecondsSinceEpoch;
      final ageDifferenceDays = ageDifferenceMs / (1000 * 60 * 60 * 24);
      expect(ageDifferenceDays.round(), equals(2),
    ...
    test('User-created notes use current time (existing behavior)', () async {
      ...
      expect(created, isNotNull);
      expect(
        created!.createdAt.isAfter(beforeCreation.subtract(const Duration(seconds: 1))),
        isTrue,
        reason: 'User-created note should use current time',
      );
      expect(
        created.createdAt.isBefore(afterCreation.add(const Duration(seconds: 1))),
        isTrue,
        reason: 'User-created note should use current time',
      );
    });

    test('REGRESSION: updateLocalNote without explicit timestamp preserves updated_at', () async {
      ...
      await harness.db.into(harness.db.localNotes).insert(
            LocalNotesCompanion.insert(
              id: noteId,
              titleEncrypted: Value(await harness.encrypt(noteId, 'Original')),
      ...
      final stored = await (harness.db.select(harness.db.localNotes)
            ..where((n) => n.id.equals(noteId)))
          .getSingle();

      expect(stored.updatedAt.toUtc().millisecondsSinceEpoch,
          originalUpdatedAt.millisecondsSinceEpoch,
          reason: 'updateLocalNote should preserve existing updated_at when not explicitly changed');
    });

    test('Explicit timestamp override works for user edits', () async {
