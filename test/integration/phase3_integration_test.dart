...
void main() {
  group('Phase 3 Integration Tests', () {
    group('Complete Dialog Flow', () {
      testWidgets('Dialog with header and actions works together', (tester) async {
    ...
    group('Task Management Flow', () {
      testWidgets('Task list with multiple display modes', (tester) async {
        ...
        // Verify priority indicator for high priority task
      ...
      testWidgets('Task interaction callbacks work correctly', (tester) async {
            ...
            home: Scaffold(
              body: TaskWidgetFactory.create(
                ...
                callbacks: TaskCallbacks(
                  onToggle: () => toggledTaskId = task.id,
                  onEdit: () => editedTaskId = task.id,
                ),
              ),
            ),
          ),
        ...
        expect(toggledTaskId, equals('test-task'));

        // Test edit tap
        await tester.tap(find.byType(InkWell).first);
        expect(editedTaskId, equals('test-task'));
      });
    });
    ...
    group('Settings Screen Flow', () {
      testWidgets('Complete settings screen with all components', (tester) async {
        bool darkMode = false;
            ...
            home: Scaffold(
              ...
              body: ListView(
                children: [
                  ...
                  const Padding(
                    padding: EdgeInsets.all(16),
                    child: Text('Folders', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                  ),
                  CompactFolderItem(
                    folder: LocalFolder(
                      id: '1',
