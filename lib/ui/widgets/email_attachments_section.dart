...
class EmailAttachmentsSection extends StatefulWidget {
  ...
  @override
  State<EmailAttachmentsSection> createState() => _EmailAttachmentsSectionState();
}
...
class _EmailAttachmentsSectionState extends State<EmailAttachmentsSection> {
  ...
  Future<void> _openAttachment(EmailAttachmentRef f) async {
    try {
      ...
      final signed = await storage.createSignedUrl(f.path, widget.signedUrlTtlSeconds);

      _cache[f.path] = _SignedUrlCache(
        url: signed,
        expiresAtMs: DateTime.now().add(Duration(seconds: widget.signedUrlTtlSeconds - 5)).millisecondsSinceEpoch,
      );
  ...
  static IconData _iconForMime(String mime) {
    ...
    if (m.contains('zip') || m.contains('compressed')) return Icons.archive_outlined;
    if (m.contains('excel') || m.contains('spreadsheet') || m.contains('xls')) return Icons.table_chart_outlined;
    if (m.contains('word') || m.contains('document') || m.contains('doc')) return Icons.description_outlined;
