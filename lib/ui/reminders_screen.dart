...
class _RemindersScreenState extends ConsumerState<RemindersScreen> {
  ...
  void initState() {
    ...
    analytics.event('screen_view', properties: {
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            Expanded(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(reminder.title, style: Theme.of(context).textTheme.titleMedium, maxLines: 1, overflow: TextOverflow.ellipsis),
                if (reminder.body != null && reminder.body!.isNotEmpty)
                  Text(reminder.body!, style: Theme.of(context).textTheme.bodySmall, maxLines: 2, overflow: TextOverflow.ellipsis),
              ]),
            ),
            ...
            PopupMenuButton<String>(
              ...
              itemBuilder: (context) => [
                ...
                if (reminder.snoozedUntil != null)
                  const PopupMenuItem(
                    value: 'unsnooze',
                    child: ListTile(leading: Icon(Icons.notifications_active), title: Text('Un-snooze'), contentPadding: EdgeInsets.zero),
                  ),
                if (reminder.isActive)
                  const PopupMenuItem(
                    value: 'deactivate',
                    child: ListTile(leading: Icon(Icons.pause), title: Text('Deactivate'), contentPadding: EdgeInsets.zero),
                  )
                else
                  const PopupMenuItem(
                    value: 'activate',
                    child: ListTile(leading: Icon(Icons.play_arrow), title: Text('Activate'), contentPadding: EdgeInsets.zero),
                  ),
                const PopupMenuItem(
                  value: 'delete',
  ...
  Widget _buildReminderStatusChip(NoteReminder reminder) {
    ...
    if (!reminder.isActive) {
      label = 'Inactive';
      color = Colors.grey;
    } else if (reminder.snoozedUntil != null && reminder.snoozedUntil!.isAfter(DateTime.now())) {
      label = 'Snoozed';
      color = Colors.orange;
    } else {
      label = 'Active';
  ...
  Future<void> _unsnoozeReminder(NoteReminder reminder) async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Reminder un-snoozed')));
      }
    } catch (e, stack) {
      logger.error('Failed to unsnooze reminder', error: e, stackTrace: stack);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Failed to unsnooze reminder')));
      }
    }
  }
  ...
  Future<void> _deactivateReminder(NoteReminder reminder) async {
    try {
      ...
      await _loadReminders();
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Reminder deactivated')));
      }
    } catch (e, stack) {
      logger.error('Failed to deactivate reminder', error: e, stackTrace: stack);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Failed to deactivate reminder')));
      }
    }
  }
  ...
  Future<void> _activateReminder(NoteReminder reminder) async {
    try {
      ...
      await _loadReminders();
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Reminder activated')));
      }
    } catch (e, stack) {
      logger.error('Failed to activate reminder', error: e, stackTrace: stack);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Failed to activate reminder')));
      }
    }
  }
          ...
          child: Column(
            children: [
              ...
              Padding(
                ...
                child: Row(children: [
                  const Expanded(child: Text('Add Reminder', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold))),
                ...
                child: Row(children: [
                  Expanded(
                    child: SegmentedButton<ReminderType>(
                      segments: const [
                        ButtonSegment(value: ReminderType.time, label: Text('Time'), icon: Icon(Icons.access_time)),
...
class _TimeReminderFormState extends ConsumerState<TimeReminderForm> {
  // Recurrence options
  static const List<String> _recurrenceOptions = ['none', 'daily', 'weekly', 'monthly', 'yearly'];
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (_recurrence != 'none') ...[
        ...
        Row(children: [
          ...
          Expanded(
            flex: 2,
            child: Text(_getIntervalLabel(_recurrence), style: Theme.of(context).textTheme.bodyLarge),
          ),
        ]),
      ],
      const SizedBox(height: 24),
      ...
      ExpansionTile(
        ...
        children: [
          TextField(
            ...
            decoration: const InputDecoration(labelText: 'Custom notification title (optional)', border: OutlineInputBorder()),
          ),
          const SizedBox(height: 16),
          TextField(
            controller: _notificationBodyController,
            decoration: const InputDecoration(labelText: 'Custom notification message (optional)', border: OutlineInputBorder()),
            maxLines: 2,
          ),
        ],
      ),
      ...
      SizedBox(
        ...
        child: ElevatedButton(
          ...
          child: _loading ? const CircularProgressIndicator() : const Text('Create Reminder'),
        ),
      ),
      const SizedBox(height: 20),
    ]);
  ...
  Future<void> _createReminder() async {
    ...
    try {
      ...
      if (reminderId != null) {
        widget.onReminderAdded();
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Reminder created successfully')));
        }
      } else {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Failed to create reminder')));
        }
      }
    } catch (e, stack) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (_hasLocation) ...[
        ...
        Row(children: [
          ...
          SizedBox(
            width: 80,
            child: Text('${_radius.round()}m', textAlign: TextAlign.center, style: Theme.of(context).textTheme.bodyLarge),
          ),
        ]),
      ],
      const SizedBox(height: 24),
      ...
      SizedBox(
        ...
        child: ElevatedButton(
          ...
          child: _loading ? const CircularProgressIndicator() : const Text('Create Location Reminder'),
        ),
      ),
      if (!_hasLocation)
        const Padding(
          padding: EdgeInsets.only(top: 8),
          child: Text('Please set a location first', style: TextStyle(color: Colors.grey), textAlign: TextAlign.center),
        ),
      const SizedBox(height: 20),
    ]);
  }
  ...
  Future<void> _createReminder() async {
    ...
    try {
      ...
      final reminderId = await coord.createLocationReminder(
        ...
        customNotificationTitle: _notificationTitleController.text.trim().isEmpty ? null : _notificationTitleController.text.trim(),
        customNotificationBody: _notificationBodyController.text.trim().isEmpty ? null : _notificationBodyController.text.trim(),
      );
      if (reminderId != null) {
        widget.onReminderAdded();
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Location reminder created successfully')));
        }
      } else {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Failed to create location reminder')));
        }
      }
    } catch (e, stack) {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        Text('Edit Reminder', style: Theme.of(context).textTheme.headlineSmall),
        const SizedBox(height: 20),
