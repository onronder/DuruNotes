...
final featureFlagsProvider = Provider<FeatureFlags>((ref) => FeatureFlags.instance);
...
final featureFlaggedPermissionManagerProvider = Provider<PermissionManager>((ref) {
  final featureFlags = ref.watch(featureFlagsProvider);
...
final featureFlaggedAdvancedReminderServiceProvider = Provider<AdvancedReminderService>((ref) {
  final featureFlags = ref.watch(featureFlagsProvider);
  final plugin = FlutterLocalNotificationsPlugin();
  final db = ref.read(appDbProvider);
...
extension FeatureFlagExtensions on WidgetRef {
  ...
  bool get useRefactoredComponents => read(featureFlagsProvider).useRefactoredComponents;
  
  /// Check if unified reminders should be used
  bool get useUnifiedReminders => read(featureFlagsProvider).useUnifiedReminders;
  ...
  bool get useUnifiedPermissionManager => read(featureFlagsProvider).useUnifiedPermissionManager;
  
  /// Get the appropriate reminder coordinator based on feature flags
  dynamic getReminderCoordinator() => read(featureFlaggedReminderCoordinatorProvider);
  
  /// Get the appropriate permission manager based on feature flags
  PermissionManager getPermissionManager() => read(featureFlaggedPermissionManagerProvider);
}
