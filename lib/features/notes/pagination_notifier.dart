...
class NotesPage {
  ...
  @override
  int get hashCode {
    return items.hashCode ^ hasMore.hashCode ^ nextCursor.hashCode ^ isLoading.hashCode;
  }
}
...
class NotesPaginationNotifier extends StateNotifier<AsyncValue<NotesPage>> {
  NotesPaginationNotifier(this._repo) 
    : super(const AsyncValue.data(NotesPage(
        items: [],
        hasMore: true,
        nextCursor: null,
      )));
  ...
  Future<void> loadMore() async {
    ...
    try {
      ...
      logger.breadcrumb('Pagination: Loading next page', data: {
      ...
      final nextCursor = newNotes.isNotEmpty ? newNotes.last.updatedAt : currentState.nextCursor;

      // Update state
      state = AsyncValue.data(NotesPage(
        items: mergedItems,
        hasMore: hasMore,
      ...
      // Analytics
      analytics.event(AnalyticsEvents.notesPageLoaded, properties: {
      ...
      logger.info('Pagination: Page loaded successfully', data: {
      ...
      logger.error('Pagination: Failed to load more notes', 
        error: error, stackTrace: stackTrace);
      
      // Revert loading state and set error
      state = AsyncValue.error(error, stackTrace);
      
      analytics.trackError('Pagination load failed', 
  ...
  Future<void> _doRefresh() async {
    ...
    try {
      // Reset state
      state = const AsyncValue.data(NotesPage(
        items: [],
        hasMore: true,
        nextCursor: null,
        isLoading: true,
      ...
      analytics.event(AnalyticsEvents.notesRefreshed, properties: {
        'trigger': 'pull_to_refresh',
      });

    } catch (error, stackTrace) {
      logger.error('Pagination: Failed to refresh notes', 
        error: error, stackTrace: stackTrace);
      state = AsyncValue.error(error, stackTrace);
    }
  }
  ...
  void checkLoadMore(double scrollPosition, double maxScrollExtent) {
    ...
    if (nearBottom) {
      analytics.event(AnalyticsEvents.notesLoadMore, properties: {
