...
class PushNotificationService {
  ...
  Future<void> initialize() async {
    try {
      ...
      _logger.info('Push notification service initialized with device ID: $_deviceId');
    } catch (e) {
  ...
  void _setupTokenRefreshListener() {
    ...
    _tokenRefreshSubscription = _messaging!.onTokenRefresh.listen(
      (newToken) async {
        ...
        final user = _client.auth.currentUser;
        if (user == null) {
          _logger.warning('Token refreshed but user not authenticated, skipping sync');
          return;
        }
  ...
  String _generateUuid() {
    ...
    final hex = random.map((byte) => byte.toRadixString(16).padLeft(2, '0')).join();
