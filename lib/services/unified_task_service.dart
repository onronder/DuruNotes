...
class UnifiedTaskService implements UnifiedTaskCallbacks {
  ...
  Future<NoteTask> createTask({
    ...
    try {
      ...
      final companion = NoteTasksCompanion.insert(
        id: taskId,
        noteId: noteId,
        content: content,
        status: Value(status),
        ...
        labels: Value(labels.isNotEmpty ? labels.join(',') : null),
        contentHash: _generateContentHash(content),
      );
      
      await _db.into(_db.noteTasks).insert(companion);
  ...
  Future<List<NoteTask>> getTasksForNote(String noteId) async {
    try {
      ...
      _logger.error('Failed to get tasks for note', 
        error: e, 
        stackTrace: stack,
        data: {'note_id': noteId},
      );
  ...
  Future<NoteTask?> getTask(String taskId) async {
    try {
      ...
      _logger.error('Failed to get task', 
        error: e, 
        stackTrace: stack,
        data: {'task_id': taskId},
      );
  ...
  Future<List<NoteTask>> getSubtasks(String parentTaskId) async {
    try {
      ...
      _logger.error('Failed to get subtasks', 
        error: e, 
        stackTrace: stack,
  ...
  Future<void> onStatusChanged(String taskId, TaskStatus newStatus) async {
    try {
      ...
      if (oldTask == null) {
        _logger.warning('Task not found for status change', data: {'task_id': taskId});
        return;
      }
      
      await (_db.update(_db.noteTasks)..where((t) => t.id.equals(taskId)))
      ...
      _logger.error('Failed to change task status', 
        error: e, 
        stackTrace: stack,
        data: {'task_id': taskId, 'new_status': newStatus.name},
      );
      _analytics.endTiming('task_status_change', properties: {'success': false});
    }
  }
  
  @override
  Future<void> onPriorityChanged(String taskId, TaskPriority newPriority) async {
    try {
      await (_db.update(_db.noteTasks)..where((t) => t.id.equals(taskId)))
        .write(NoteTasksCompanion(
      ...
      _logger.error('Failed to change task priority', 
        error: e, 
        stackTrace: stack,
    ...
    try {
      await (_db.update(_db.noteTasks)..where((t) => t.id.equals(taskId)))
        .write(NoteTasksCompanion(
          ...
          contentHash: Value(_generateContentHash(newContent)),
          updatedAt: Value(DateTime.now()),
        ));
      ...
      _logger.error('Failed to change task content', 
        error: e, 
        stackTrace: stack,
        data: {'task_id': taskId},
      );
  ...
  Future<void> onDeleted(String taskId) async {
    try {
      ...
      // Delete the task
      await (_db.delete(_db.noteTasks)..where((t) => t.id.equals(taskId))).go();
      ...
      _logger.error('Failed to delete task', 
        error: e, 
        stackTrace: stack,
        data: {'task_id': taskId},
      );
  ...
  Future<void> onDueDateChanged(String taskId, DateTime? newDate) async {
    try {
      ...
      _logger.error('Failed to change task due date', 
        error: e, 
        stackTrace: stack,
        data: {'task_id': taskId},
      );
  ...
  Future<void> batchUpdateStatus(List<String> taskIds, TaskStatus newStatus) async {
    try {
      ...
      _logger.error('Failed to batch update tasks', 
        error: e, 
        stackTrace: stack,
  ...
  Future<List<NoteTask>> searchTasks(String query) async {
    try {
      ...
      _logger.error('Failed to search tasks', 
        error: e, 
        stackTrace: stack,
        data: {'query': query},
      );
  ...
  Future<TaskStatistics> getTaskStatistics() async {
    try {
      ...
      final cancelled = allTasks.where((t) => t.status == TaskStatus.cancelled).length;
      
      final overdue = await getOverdueTasks();
      
      final byPriority = <TaskPriority, int>{};
      for (final priority in TaskPriority.values) {
        byPriority[priority] = allTasks.where((t) => t.priority == priority).length;
      }
...
final tasksForNoteProvider = FutureProvider.family<List<NoteTask>, String>((ref, noteId) {
  final service = ref.watch(unifiedTaskServiceProvider);
