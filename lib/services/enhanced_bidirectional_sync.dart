...
class EnhancedBidirectionalSync {
  ...
  Future<void> initializeSync(String noteId) async {
    try {
      ...
      _logger.error('Failed to initialize enhanced sync',
        error: e,
        stackTrace: stack,
        data: {'noteId': noteId},
      );
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      if (taskInfo != null) {
        ...
        if (embeddedId == null) {
          ...
          final existingTask = tasksByContent[taskInfo.content.toLowerCase().trim()];
  ...
  TaskInfo? _parseTaskLine(String line) {
    ...
    final cleanLine = line.replaceAll(RegExp(r'\s*<!-- task-id:[a-f0-9\-]+ -->'), '');
    ...
    // Extract priority
    final priorityMatch = RegExp(r'#(low|medium|high|urgent)').firstMatch(content);
  ...
  Future<void> syncFromNoteToTasks(String noteId, String noteContent) async {
    ...
    try {
      ...
      _logger.error('Enhanced sync failed',
        error: e,
        stackTrace: stack,
        data: {'noteId': noteId},
      );
