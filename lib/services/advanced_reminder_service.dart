...
import 'package:duru_notes/core/providers/infrastructure_providers.dart'
    show loggerProvider, analyticsProvider, supabaseClientProvider;
...
class AdvancedReminderService {
  ...
  Future<void> init() async {
    ...
    try {
      ...
      analytics.event(
        'app.feature_enabled',
        properties: {'feature': 'advanced_reminders'},
      );
    } catch (e, stack) {
      logger.error(
        'Failed to initialize AdvancedReminderService',
        error: e,
        stackTrace: stack,
      );
      analytics.event(
  ...
  Future<bool> requestNotificationPermissions() async {
    try {
      ...
      analytics.event(
        granted
            ? AnalyticsEvents.reminderPermissionGranted
            : AnalyticsEvents.reminderPermissionDenied,
        properties: {
          'type': 'notification',
...
final advancedReminderServiceProvider = Provider<AdvancedReminderService>((
  ref,
) {
