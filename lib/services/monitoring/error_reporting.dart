...
class ErrorReportingService {
  ...
  ErrorAnalysis analyzeErrors({
    ...
    final errorRate = relevantErrors.length / 
        (timeWindow?.inMinutes ?? 1);
  ...
  Future<bool> attemptRecovery({
    ...
    for (int attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        _logger.info('Attempting error recovery', data: {
          'attempt': attempt,
        ...
        if (success) {
          _logger.info('Error recovery successful', data: {
            'attempt': attempt,
            'error_type': error.runtimeType.toString(),
          });
        ...
        _logger.warning('Recovery attempt failed', error: e, data: {
          'attempt': attempt,
  ...
  Map<String, dynamic> _buildSentryExtra(ErrorReport report) {
    return {
      'category': report.category.name,
      'severity': report.severity.name,
      if (report.sourceLocation != null) 'source_location': report.sourceLocation,
