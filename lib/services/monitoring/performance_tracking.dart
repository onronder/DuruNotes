...
class PerformanceTrackingService {
  PerformanceTrackingService({required this.sentryMonitoring});
  ...
  ISentrySpan trackAppStartup() {
    final transaction = sentryMonitoring.startTransaction(
      ...
      data: {'cold_start': true, 'timestamp': DateTime.now().toIso8601String()},
    );
  ...
  ISentrySpan trackNavigation({
    ...
    final transaction = sentryMonitoring.startTransaction(
      ...
      data: {'from': from, 'to': to, ...?extra},
    );

    // Add navigation breadcrumb
    sentryMonitoring.addNavigationBreadcrumb(from: from, to: to, data: extra);
  ...
  void _logSlowOperation({
    ...
    unawaited(
      sentryMonitoring.reportMessage(
        message:
            'Slow $type operation: $operation took ${duration.inMilliseconds}ms',
...
class PerformanceMetric {
  PerformanceMetric({required this.name, required this.category});
...
/// Metric categories
enum MetricCategory { lifecycle, navigation, database, network, ui, custom }
