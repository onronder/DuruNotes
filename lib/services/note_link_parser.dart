...
class NoteLinkParser {
  ...
  static final _atMentionRegex = RegExp(
    r'@\[([^\]]+)\]',
    multiLine: true,
  );
  ...
  Future<List<Map<String, String?>>> extractLinks(
    ...
    try {
      ...
      _logger.debug('[NoteLinkParser] Extracting links from content', data: {
      ...
      for (final match in atMentions) {
        ...
        if (title != null && title.isNotEmpty && !processedTitles.contains(title.toLowerCase())) {
      ...
      _logger.info('[NoteLinkParser] Extracted links', data: {
      ...
      _logger.error('[NoteLinkParser] Failed to extract links', error: e, stackTrace: stack);
      return [];
    }
  }
    ...
    try {
      ...
      for (final note in allNotes) {
        if (note.title.toLowerCase().trim() == titleLower) {
          _logger.debug('[NoteLinkParser] Found exact match', data: {
            'title': title,
            'note_id': note.id,
          });
          return note.id;
      ...
      for (final note in allNotes) {
        if (note.title.toLowerCase().contains(titleLower)) {
          _logger.debug('[NoteLinkParser] Found fuzzy match', data: {
            'title': title,
      ...
      _logger.warning('[NoteLinkParser] No note found for title', data: {'title': title});
      return null;
    } catch (e, stack) {
      _logger.error('[NoteLinkParser] Error finding note by title', error: e, stackTrace: stack);
      return null;
    }
  }
  ...
  Future<List<domain.Note>> searchNotesByTitle(
    ...
    try {
      ...
      // Then fuzzy matches
      for (final note in allNotes) {
        if (!matches.contains(note) && note.title.toLowerCase().contains(titleLower)) {
          matches.add(note);
        }
      }
      ...
      _logger.debug('[NoteLinkParser] Search results', data: {
        'query': partialTitle,
      ...
      _logger.error('[NoteLinkParser] Error searching notes', error: e, stackTrace: stack);
      return [];
    }
  }
  ...
  Future<List<domain.Note>> getBacklinks(
    ...
    try {
      if (_noteIndexer != null) {
        ...
        if (backlinkIds.isNotEmpty) {
          ...
          if (backlinks.isNotEmpty) {
            _logger.info('[NoteLinkParser] Backlinks (index)', data: {
              'note_id': noteId,
      ...
      _logger.info('[NoteLinkParser] Found backlinks', data: {
        'note_id': noteId,
        'backlink_count': backlinks.length,
      });
      ...
      _logger.error('[NoteLinkParser] Error getting backlinks', error: e, stackTrace: stack);
      return [];
    }
  }
  ...
  Future<String> replaceLinksWithMarkdown(
    ...
    try {
      ...
      _logger.error('[NoteLinkParser] Error replacing links', error: e, stackTrace: stack);
      return content;
    }
  }
