...
class EmailAliasService {
  ...
  Future<String?> getOrCreateAlias() async {
    try {
      ...
      if (cachedAlias != null) {
        _logger.debug('[EmailAliasService] Using cached alias: $cachedAlias');
        return cachedAlias;
      }
      ...
      if (response != null && response['alias'] != null) {
        ...
        _logger.debug('[EmailAliasService] Retrieved existing alias: $alias');
        return alias;
      }
      ...
      if (result != null) {
        ...
        await _cacheAlias(userId, alias);
        _logger.debug('[EmailAliasService] Generated new alias: $alias');
        return alias;
      }

      return null;
    } catch (e) {
      _logger.debug('[EmailAliasService] Error getting alias: $e');
      return null;
    }
  }
  ...
  Future<String?> getFullEmailAddress() async {
    ...
    if (alias == null) {
      _logger.debug('[EmailAliasService] No alias available');
      return null;
    }
    final domain = inboundDomain;
    final fullAddress = '$alias@$domain';
    _logger.debug('[EmailAliasService] Full email address: $fullAddress');
    return fullAddress;
  }
  ...
  Future<void> clearCache() async {
    try {
      ...
      _logger.debug('[EmailAliasService] Cache cleared for user: $userId');
    } catch (e) {
      _logger.debug('[EmailAliasService] Error clearing cache: $e');
    }
  }
    ...
    try {
      ...
      if (timestamp != null) {
        ...
        if (DateTime.now().difference(cachedTime) > _cacheExpiration) {
          _logger.debug('[EmailAliasService] Cache expired for user: $userId');
          return null;
        }
      }
      ...
      _logger.debug('[EmailAliasService] Error reading cache: $e');
      return null;
    }
  }
  ...
  Future<void> _cacheAlias(String userId, String alias) async {
    try {
      ...
      _logger.debug('[EmailAliasService] Cached alias for user: $userId');
    } catch (e) {
      _logger.debug('[EmailAliasService] Error caching alias: $e');
    }
  }
}
