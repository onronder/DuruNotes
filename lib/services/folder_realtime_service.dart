...
class FolderRealtimeService {
    ...
    _connectivitySubscription = Connectivity().onConnectivityChanged.listen(
      (List<ConnectivityResult> results) {
        ...
        if (hasConnection && _state == ConnectionState.offline) {
          debugPrint('[FolderRealtime] Network restored, attempting reconnection');
          _setState(ConnectionState.disconnected);
  ...
  Future<void> _connect() async {
    ...
    try {
      debugPrint('[FolderRealtime] Connecting for user $_currentUserId (attempt ${_reconnectAttempts + 1})');
  ...
  void _onSubscriptionStatusChanged(RealtimeSubscribeStatus status, [Object? error]) {
