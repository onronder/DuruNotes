...
class CacheManager {
  ...
  Future<void> initialize() async {
    ...
    _logger.info('CacheManager initialized', data: {
  ...
  Future<T?> get<T>(String key) async {
    ...
    if (_prefs != null) {
      ...
      if (persistedJson != null) {
        try {
          final entry = CacheEntry.fromJson(jsonDecode(persistedJson));
          if (!entry.isExpired) {
            _stats.l2Hits++;
          ...
          _logger.warning('Failed to deserialize cache entry', error: e);
        }
      }
    }
    ...
    if (level == CacheLevel.all || level == CacheLevel.persistent) {
      if (_prefs != null && _shouldPersist(value)) {
        try {
          ...
          _logger.warning('Failed to persist cache entry', error: e);
        }
      }
    }
  ...
  Future<void> invalidatePattern(String pattern) async {
    ...
    if (_prefs != null) {
      ...
      final cacheKeys = allKeys.where((key) => key.startsWith('cache_') && regex.hasMatch(key.substring(6)));
  ...
  Future<void> _cleanupExpiredEntries() async {
    ...
    if (_prefs != null) {
      ...
      for (final key in cacheKeys) {
        ...
        if (json != null) {
          try {
            final entry = CacheEntry.fromJson(jsonDecode(json));
            if (entry.isExpired) {
              await _prefs!.remove(key);
            }
          } catch (e) {
...
class FolderHierarchyCache {
  ...
  Future<List<LocalFolder>?> getRootFolders() async {
    ...
    if (cached != null) {
      return cached.map((json) => LocalFolder.fromJson(json as Map<String, dynamic>)).toList();
    }
    return null;
  }
  ...
  Future<List<LocalFolder>?> getChildFolders(String parentId) async {
    final cached = await _cacheManager.get<List<dynamic>>('$_folderChildrenPrefix$parentId');
    if (cached != null) {
      return cached.map((json) => LocalFolder.fromJson(json as Map<String, dynamic>)).toList();
    }
    return null;
  ...
  Future<void> setChildFolders(String parentId, List<LocalFolder> folders) async {
    await _cacheManager.put(
