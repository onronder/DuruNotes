...
class PerformanceMonitor {
  ...
  void endTransaction(String operation, {SpanStatus? status}) {
    ...
    if (timer != null) {
      ...
      _logger.debug('Ended transaction: $operation (${timer.elapsedMilliseconds}ms)');
    }
  }
  ...
  Future<T> measure<T>(String operation, Future<T> Function() action) async {
    ...
    try {
      ...
      stopwatch.stop();
      _metrics.recordOperation(operation, stopwatch.elapsedMilliseconds, success: false);
      rethrow;
    }
  }
  ...
  void _updateMemoryMetrics() {
    ...
    if (_memorySnapshots.length >= 3) {
      final growth = latest.usedMemoryBytes - _memorySnapshots.first.usedMemoryBytes;
      final duration = latest.timestamp.difference(_memorySnapshots.first.timestamp);
      
      if (duration.inMinutes > 0) {
        final growthRate = growth / duration.inMinutes; // Bytes per minute
        
        if (growthRate > 1024 * 1024) { // More than 1MB per minute
...
class PerformanceMetrics {
  ...
  void recordOperation(String operation, int durationMs, {bool success = true}) {
    operationCount++;
...
class MemorySnapshot {
  ...
  double get usagePercentage => totalMemoryBytes > 0 
...
class PerformanceReport {
  ...
  List<String> _getRecommendations() {
    ...
    if (metrics.droppedFrameRate > 0.05) {
      recommendations.add('High frame drop rate detected (${(metrics.droppedFrameRate * 100).toStringAsFixed(1)}%). Consider optimizing heavy UI operations.');
    }
    
    if (metrics.memoryUsageMB > 200) {
      recommendations.add('High memory usage (${metrics.memoryUsageMB.toStringAsFixed(0)}MB). Consider clearing caches or optimizing data structures.');
    }
    
    if (metrics.slowOperationCount > 10) {
      recommendations.add('Multiple slow operations detected (${metrics.slowOperationCount}). Review operation performance.');
    }
    ...
    if (slowestOp.value.maxDuration > 5000) {
      recommendations.add('Operation "${slowestOp.key}" took ${slowestOp.value.maxDuration}ms. Consider optimizing or adding progress indicators.');
    }
