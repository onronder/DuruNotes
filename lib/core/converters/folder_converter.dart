...
class FolderConverter {
  ...
  static LocalFolder toLocal(domain.Folder folder) {
    return LocalFolder(
      ...
      deleted:
          false, // Domain folders don't have deleted field, assume not deleted
    );
  }

  /// Convert `List<LocalFolder>` to `List<domain.Folder>`
  static List<domain.Folder> fromLocalList(
    List<LocalFolder> localFolders, {
    String? userId,
  }) {
    return localFolders
        .map((local) => fromLocal(local, userId: userId))
        .toList();
  }
