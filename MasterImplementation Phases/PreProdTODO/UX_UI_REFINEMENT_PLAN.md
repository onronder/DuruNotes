...
- **File**: `lib/services/analytics/analytics_service.dart`
  - [ ] Ensure analytics events exist for:
...
- **File**: `lib/ui/notes_list_screen.dart`
  - [ ] Track which FAB mini-actions are used (log events when tapped).

- **File**: `lib/ui/screens/modern_home_screen.dart`
  - [ ] Track FAB usage similarly.

### 1.2 Short-Term Internal Usage Logging

- **File**: `docs/UX_AUDIT_NOTES.md` (NEW)
  - [ ] Capture findings after a short internal usage period:
...
- **File**: `lib/ui/notes_list_screen.dart`
  ...
  - [ ] Remove or relocate rarely used actions from the FAB into menus or other entry points.
...
- **File**: `lib/ui/screens/modern_home_screen.dart`
  ...
  - [ ] Avoid duplicate or conflicting labels between screens.
...
- **File**: `lib/ui/modern_edit_note_screen.dart`
  - [ ] List all toolbar buttons (bold, italic, highlight, attachments, voice dictation, etc.).
  - [ ] Group rarely-used formatting into a secondary “More” menu or bottom sheet.
...
- **File**: `lib/ui/modern_edit_note_screen.dart`
  - [ ] Keep context menus focused on actions relevant to the current note (e.g. share, secure share, move to folder).
  - [ ] Move global actions (e.g. app settings) out of this screen and into the main navigation.
...
- **File**: `lib/ui/settings_screen.dart`
  - [ ] Group settings into clear sections:
    ...
    - Voice & Audio
    - Security & Privacy
  - [ ] Move advanced/rarely-used toggles from scattered screens into Settings:
    - E.g. some debug or advanced search options, where appropriate.

### 4.2 Voice & OCR Section

- **File**: `lib/ui/settings_screen.dart`
  - [ ] Add a “Voice & OCR” section that links to:
...
- **File**: `lib/ui/help_screen.dart`
  - [ ] Ensure “Voice & OCR Capture” section is up-to-date:
    - Document new voice note and dictation flows.
  - [ ] Keep sections concise; move detailed content to docs or the website if it’s too long.
...
- **Files**: `lib/ui/widgets/*`
  - [ ] Identify and extract common patterns (e.g., tile layouts, action rows) into shared widgets.
  - [ ] Use consistent iconography for similar actions (mic, share, trash).
...
- [ ] FABs present only primary, high-value actions.
- [ ] Editor toolbar is not overloaded; advanced options are discoverable but not noisy.
- [ ] Settings are grouped logically; advanced options aren’t scattered.
- [ ] Voice note and dictation flows are easy to discover and understand.
- [ ] Help text matches actual behavior.
- [ ] Accessibility standards are maintained for primary flows.
