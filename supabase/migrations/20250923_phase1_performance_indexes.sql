...
-- Notes table indexes
CREATE INDEX IF NOT EXISTS idx_notes_user_updated_covering
ON notes(user_id, updated_at DESC, id, is_pinned)
WHERE deleted = false;

CREATE INDEX IF NOT EXISTS idx_notes_user_sync_recent
...
CREATE INDEX IF NOT EXISTS idx_note_tasks_user_due_pending
...
CREATE INDEX IF NOT EXISTS idx_folders_user_parent_covering
ON folders(user_id, parent_id, sort_order, id)
WHERE deleted = false;

-- Note-folder relationship indexes
CREATE INDEX IF NOT EXISTS idx_note_folders_user_covering
...
-- Templates indexes
CREATE INDEX IF NOT EXISTS idx_templates_user_category_usage
ON templates(user_id, category, usage_count DESC, sort_order)
WHERE deleted = false;

-- Clipper inbox indexes
CREATE INDEX IF NOT EXISTS idx_clipper_inbox_user_unread_recent
